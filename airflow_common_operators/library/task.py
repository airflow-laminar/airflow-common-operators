...
class LibraryOperatorBase(PythonOperator):
    ...
    def __init__(self, task_id: str, pip: List[PipLibrary], git: List[GitRepo], operator, operator_command_arg, operator_args, **kwargs):
        self.pip = pip
        self.git = git
        ...
        for git_repo in self.git:
            git_repo_clone_op = self.operator(
                ...
                **{operator_command_arg: git_repo.clone()},
                **kwargs,
            )
        ...
        for pip_lib in self.pip:
            pip_lib_install_op = self.operator(
                ...
                **{operator_command_arg: pip_lib.install()},
                **kwargs,
            )
            # NOTE: overloading shift so use parent's
...
class InstallLibraryOperator(LibraryOperatorBase):
    def __init__(self, task_id: str, pip: List[PipLibrary], git: List[GitRepo], **bash_args):
        from airflow.providers.standard.operators.bash import BashOperator

        obj = LibraryListTaskArgs.model_validate({"pip": pip, "git": git})

        super().__init__(
            task_id=task_id, pip=obj.pip, git=obj.git, operator=BashOperator, operator_command_arg="bash_command", operator_args=bash_args
        )


class InstallLibrarySSHOperator(LibraryOperatorBase):
    def __init__(self, task_id: str, pip: List[PipLibrary], git: List[GitRepo], **ssh_args):
        from airflow.providers.ssh.operators.ssh import SSHOperator

        obj = LibraryListSSHTaskArgs.model_validate({"pip": pip, "git": git})

        super().__init__(task_id=task_id, pip=obj.pip, git=obj.git, operator=SSHOperator, operator_command_arg="command", operator_args=ssh_args)
